          ...
          <tbody>
            ...
            {orders.map((order) => (
              <tr key={order._id}>
                ...
                <td>{order.user && order.user.name}</td> {/* Display user name if available */}
                <td>{order.createdAt.substring(0, 10)}</td> {/* Display order creation date */}
                <td>{order.totalPrice}</td> {/* Display total price of the order */}
                <td>
                  {order.isPaid ? (
